...
class Command(BaseCommand):
    help = 'Generates 200 rows of fake data using Faker for testing and an admin user'
    ...
    def handle(self, *args, **kwargs):
        ...
        # 0. Create Admin User
        admin_qs = User.objects.filter(phone_number='admin@admin.com')
        if not admin_qs.exists():
            User.objects.create_superuser(phone_number='admin@admin.com', password='123', is_admin=True, is_staff=True)
            self.stdout.write(self.style.SUCCESS("Admin user created: admin@admin.com / 123"))
        else:
